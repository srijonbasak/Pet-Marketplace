...
const EmployeeDashboard = () => {
    ...
    const fetchEmployeeData = async () => {
      try {
        ...
        if (employeeRes.data.permissions.canAddProducts || employeeRes.data.permissions.canManageInventory) {
          const productsRes = await api.get(`/products?shop=${employeeRes.data.shop}`, config);
        ...
        if (employeeRes.data.permissions.canCreateInvoices) {
          try {
            ...
            const invoicesRes = await api.get('/invoices/shop', config);
  ...
  const handlePaymentStatusChange = async (invoiceId, newStatus) => {
    try {
      ...
      await api.put(`/invoices/${invoiceId}/status`, { paymentStatus: newStatus }, config);
