...
const Home = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      {/* Hero Section */}
      <div className="hero-section text-white text-center py-5 position-relative">
        <InteractivePetBackground />
        <Container className="position-relative" style={{ zIndex: 1 }}>
          ...
          <motion.div className="d-grid gap-3 d-md-flex justify-content-md-center" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5, delay: 0.4, type: 'spring' }}>
      ...
      <div className="py-5 bg-light full-width-section">
        <Container>
          ...
          <Row>
            {[
              { icon: faPaw, title: 'Pet Adoption', text: 'Find your perfect companion from our selection of loving pets.', link: '/pets' },
              { icon: faShoppingCart, title: 'Pet Products', text: 'High-quality food, toys, and accessories for your furry friends.', link: '/products' },
              { icon: faHandHoldingHeart, title: 'Support Rescue', text: 'Help us save animals in need through donations and support.', link: '/rescues' },
            ].map((service, index) => (
              <Col md={4} className="mb-4 text-center" key={index}>
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.2 }}>
                  <div className="service-icon-wrapper mx-auto mb-4">
